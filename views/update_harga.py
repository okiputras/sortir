...
manual = S.load_mapping(sh)
plan = S.plan_updates(rows, catalog, manual)
...
if manual:
    st.caption(f"Mapping manual aktif: {len(manual)} entri (tab {S.MAPPING_TAB}).")
...
with st.expander(f"Belum cocok ({len(unmatched)}) — dibiarkan apa adanya"):
    st.dataframe(
        pd.DataFrame([{"Nama": p["nama"], "Tipe": p["tipe"]} for p in unmatched]),
        use_container_width=True, hide_index=True,
    )
    st.caption(f"Untuk mencocokkan manual, isi tab **{S.MAPPING_TAB}** "
               "(kolom: nama_barang_edit | nama_olshopin).")
